...
JNIEXPORT jint JNICALL Java_com_topeet_ledtest_led_Open
  ...
  {
	if(fd<=0)fd = open("/dev/leds_ctl", O_RDWR|O_NDELAY|O_NOCTTY);
	if(fd <=0 )__android_log_print(ANDROID_LOG_INFO, "serial", "open /dev/leds_ctl Error");
	else __android_log_print(ANDROID_LOG_INFO, "serial", "open /dev/leds_ctl Sucess fd=%d",fd);
  }
